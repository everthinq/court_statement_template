...
def process_data(request):
    ...
    if 1 in payment_doc_type:
        ...
        for i in range(1, platejnoe_poruchenie_extras_counter):
            if data['extra_plat_por_num_' + str(i)] != '' and data['extra_plat_por_date_' + str(i)] != '':
                ...
                pl_poruch_extra_payment_docs.append("Копия платежного поручения №%s от %s г" % (doc_number, doc_date))
    ...
    if 2 in payment_doc_type:
        ...
        kvitanciya_extras_counter = data['extras_kvitanciya']
        for i in range(1, kvitanciya_extras_counter):
            if data['extra_order_num_' + str(i)] != '' and data['extra_step4_rb2_input2_' + str(i)] != '':
                doc_number = data['extra_order_num_' + str(i)]
                doc_date = datetime.strptime(data['extra_step4_rb2_input2_' + str(i)], '%d/%m/%Y')
                doc_date = datetime.strftime(doc_date, '%d.%m.%Y')
                payment_docs.append("квитанцией к приходному кассовому ордеру №%s от %s" % (doc_number, doc_date))
                kvitanciya_extra_payment_docs.append("Копия приходного кассового ордера №%s от %s г" % (doc_number, doc_date))
    ...
    if 4 in payment_doc_type:
        ...
        akkreditiv_extras_counter = data['extras_akkreditiv']
        for i in range(1, akkreditiv_extras_counter):
            if data['extra_akkreditivNUM_' + str(i)] != '' and data['extra_akkreditivDATE_' + str(i)] != '':
                doc_number = data['extra_akkreditivNUM_' + str(i)]
                doc_date = datetime.strptime(data['extra_akkreditivDATE_' + str(i)], '%d/%m/%Y')
                doc_date = datetime.strftime(doc_date, '%d.%m.%Y')
                payment_docs.append("аккредитивом №%s от %s" % (doc_number, doc_date))
                akkreditiv_extra_payment_docs.append("Копия аккредитива №%s от %s г" % (doc_number, doc_date))
    ...
    if data['overdue_days'] > 0:
        ...
        data['penalty_and_rent'] = penalty + float(data['rent_total'])
